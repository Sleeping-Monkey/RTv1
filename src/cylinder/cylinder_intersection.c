...
/*   Updated: 2019/08/29 17:05:18 by ssheba           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	cylinder_intersection(void *data, t_ray *view, t_vec3 *pos, t_real *t)
{
	...
	v3_sub(m3v3_mul(&cylinder->axis, &view->start, &axis_view.start), \
	&cylinder->o, vec + 0);
	v3_sub(m3v3_mul(&cylinder->axis, &view->finish, &axis_view.finish), \
	&axis_view.start, vec + 1);
	sc[0] = vec[1].x * vec[1].x + vec[1].y * vec[1].y;
	sc[1] = vec[0].x * vec[1].x + vec[0].y * vec[1].y;
	sc[2] = vec[0].x * vec[0].x + vec[0].y * vec[0].y - cylinder->r_2;
	...
	if (t)
		*t = sc[4];
	m3v3_mul(&cylinder->inv_axis, v3_add(&axis_view.start, \
